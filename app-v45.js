window.FW_CORE_LOADED = "v47";
window.FW_APP_LOADED = "v45";
try {
  const st = document.getElementById("fw-status");
  if (st) st.innerText = "App loaded v45…";
  const v = document.getElementById("fw-version");
  if (v) v.textContent = "v45";
} catch(e) {}
...
async function init(){
  setFooterVersion();
  setStatusSafe("Scanning airspace…");
...
async function tick(){
  ...
  try{
    await loadFlights();
    await renderFlight();
    // If renderFlight doesn't update status, keep it helpful
    const st = document.getElementById("fw-status");
    if (st && (st.innerText||"").includes("Scanning airspace")) {
      // renderFlight/loadFlights will update status when data arrives; otherwise leave as-is
    }
  }catch(e){
    // Never stall; show short status and keep looping
    setStatusSafe("Radar temporarily unavailable");
  }finally{
    tickInFlight = false;
  }
}
  ...
  // Start heartbeat loop immediately (never block startup)
  startHeartbeat();
}
...
// ===== v47 forced init (script may load after DOMContentLoaded) =====
(function(){
  function setStatus(msg){
    try{ var st=document.getElementById('fw-status'); if(st) st.innerText=msg; }catch(e){}
    try{ var v=document.getElementById('fw-version'); if(v) v.textContent='v47'; }catch(e){}
  }

  try{
    if (typeof init === 'function'){
      setStatus('Starting app v47…');
      // If init is async, fire and forget
      Promise.resolve(init()).catch(function(e){ setStatus('INIT ERROR: ' + (e && e.message ? e.message : String(e))); });
    } else {
      setStatus('ERROR: init() not found (v47)');
    }
  }catch(e){
    setStatus('INIT EXCEPTION: ' + (e && e.message ? e.message : String(e)));
  }
})();
