...
function start(){
  document.getElementById("fw-version").textContent = "v69";
  hookControls();
...
function updateSecondaryBoxes(queue, currentIndex){
  ...
  for (let i=0;i<4;i++){
    ...
    // Stats
    const dist = isFinite(f.dist) ? `${Math.round(f.dist)}MI` : "—";
    const brg = isFinite(f.bearing) ? compassFromBearing(f.bearing) : (isFinite(f.brg) ? compassFromBearing(f.brg) : "—");
    const alt = isFinite(f.alt_ft) ? `${Math.round(f.alt_ft)}FT` : (isFinite(f.alt) ? `${Math.round(f.alt)}FT` : "—");
    const spd = isFinite(f.spd_mph) ? `${Math.round(f.spd_mph)}MPH` : (isFinite(f.spd) ? `${Math.round(f.spd)}MPH` : "—");
