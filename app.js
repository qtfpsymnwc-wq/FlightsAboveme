...
// v132: adaptive radar polling to avoid OpenSky 429
let __fwRadarPollMs = 5000;      // start at 5s
const __fwRadarPollMinMs = 4000; // floor
const __fwRadarPollMaxMs = 60000;// ceiling 60s
let __fwRadarConsecErr = 0;
function __fwRadarBackoff(hit429){
  __fwRadarConsecErr = Math.min(10, __fwRadarConsecErr + 1);
  const factor = hit429 ? 2.2 : 1.6;
  __fwRadarPollMs = Math.min(__fwRadarPollMaxMs, Math.max(__fwRadarPollMinMs, Math.round(__fwRadarPollMs * factor)));
}
function __fwRadarRecover(){
  __fwRadarConsecErr = 0;
  __fwRadarPollMs = Math.max(__fwRadarPollMinMs, Math.round(__fwRadarPollMs * 0.85));
}
...
(function(){
  const AERO_PROXY_ORIGIN = () => FW_PROXY_ORIGIN();
  ...
  async function fetchAircraftByIcao24(icao24){
    ...
    const __aeroOrigin = (typeof AERO_PROXY_ORIGIN === "function") ? AERO_PROXY_ORIGIN() : AERO_PROXY_ORIGIN;
    const base = String(__aeroOrigin||"").replace(/\/$/,"");
...
if (!window.__FW_ROUTE_WIRED__) {
  ...
  (function(){
    const _AERO_ORIGIN = () => FW_PROXY_ORIGIN();
...
function _now(){ return Date.now(); }
    ...
    async function _fetch(cs){
      ...
      const __routeOrigin = (typeof _AERO_ORIGIN === "function") ? _AERO_ORIGIN() : _AERO_ORIGIN;
      const base = String(__routeOrigin||"").replace(/\/$/,"");
...
/* v132: proxy origin auto-detect (no manual edits) */
const FW_FALLBACK_PROXY = "https://flightsabove.t2hkmhgbwz.workers.dev/";
window.__FW_PROXY_ORIGIN__ = window.__FW_PROXY_ORIGIN__ || FW_FALLBACK_PROXY;

function __fwNormProxy(origin){
  try{
    const u = new URL(origin);
    return u.origin + "/";
  }catch(e){
    const s = String(origin||"").trim();
    if (!s) return FW_FALLBACK_PROXY;
    return s.endsWith("/") ? s : (s + "/");
  }
}
function FW_PROXY(){
  return __fwNormProxy(window.__FW_PROXY_ORIGIN__ || FW_FALLBACK_PROXY);
}
function FW_PROXY_ORIGIN(){
  const p = FW_PROXY();
  return p.endsWith("/") ? p.slice(0,-1) : p;
}
async function __fwFetchWithTimeout(url, ms){
  const ctrl = new AbortController();
  const t = setTimeout(()=>ctrl.abort(), ms);
  try{ return await fetch(url, { signal: ctrl.signal, cache: "no-store" }); }
  finally{ clearTimeout(t); }
}
async function __fwProbe(origin){
  const base = __fwNormProxy(origin);
  const res = await __fwFetchWithTimeout(base + "health", 2200);
  if (!res.ok) return null;
  let js = null;
  try{ js = await res.json(); }catch(e){}
  if (js && js.ok) return base;
  return null;
}
(async function(){
  // Candidates: same-origin (if you host the site on the Worker), last-known, and fallback.
  const last = (function(){ try{ return localStorage.getItem("fw.proxy") || ""; }catch(e){ return ""; } })();
  const cands = [location.origin, last, FW_FALLBACK_PROXY].filter(Boolean);
  for (const c of cands){
    try{
      const ok = await __fwProbe(c);
      if (ok){
        window.__FW_PROXY_ORIGIN__ = ok;
        try{ localStorage.setItem("fw.proxy", ok); }catch(e){}
        break;
      }
    }catch(e){}
  }
...
async function loadFlights(){
  ...
  const url = `${FW_PROXY()}opensky/states?lamin=${lamin}&lomin=${lomin}&lamax=${lamax}&lomax=${lomax}`;
  const res = await fetchWithTimeout(url, 6500);
...
function start(){
  document.getElementById("fw-version").textContent = "v132";
  hookControls();
