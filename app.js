...
function airlineCodeFromCallsign(callsign){
  ...
  // Only accept ICAO-style 3-letter prefix at start (letters only).
  // Blocks short/odd callsigns like SD1.
...
function isCommercialCallsign(callsign){
  ...
  const cs = String(callsign).trim().toUpperCase();
  // Private/GA tail numbers (common US)
  if (/^N\d/.test(cs)) return false;
  const pref = airlineCodeFromCallsign(cs);
  // Require ICAO-style 3-letter airline code (this blocks SD1 and similar)
...
function render(){
  try{ applyFiltersAndLimit(); }catch(e){}
...
function start(){
  document.getElementById("fw-version").textContent = "v81";
  hookControls();
