...
function formatRouteWithCities(routeText) {
  ...
  const m = routeText.match(/^([A-Z]{3})\s*→\s*([A-Z]{3})$/);
...
async function loadFlights() {
  ...
  try {
    const url = `${PROXY}opensky/states?lamin=${HOME_LAT-RANGE}&lomin=${HOME_LON-RANGE}&lamax=${HOME_LAT+RANGE}&lomax=${HOME_LON+RANGE}`;
