...
function setRouteLine(text){
  ...
  if (!t){
    el.textContent = "";
    el.classList.remove("showRoute");
    return;
  }
  el.textContent = t;
  el.classList.add("showRoute");
}
...
async function fetchRouteTextForCallsign(callsign){
  ...
  try{
    ...
    if (!txt && j && j.origin && j.destination){
      ...
      const oc = (o.municipalityName || o.city || o.name || "");
      const dc = (d.municipalityName || d.city || d.name || "");
...
async function loadFlights(){
  ...
  try{ setStatusLine(`Radar: ${rawCount} flights • Showing: ${flights.length}`); }catch(e){}
  current = pickCurrent();
...
function start(){
  document.getElementById("fw-version").textContent = "v97";
  hookControls();
