...
function fwGetBboxV118(lat, lon){
...
try{ window.__FW_FORCE_ROUTE__ = update; }catch(e){}
})();
...
if (!window.__FW_ROUTE_WIRED__) {
  ...
  (function(){
    ...
    const _lastGoodRoute = new Map(); // callsign -> last known good route string
    ...
    async function update(){
      try{
        ...
        try{
          ...
          if (cached && cached.route) { const r = __fwFixArrow(cached.route); _set(r); _lastGoodRoute.set(cs, r); }
                else {
          const lg = _lastGoodRoute.get(cs);
          if (lg) _set(lg);
          else _set("EN ROUTE");
        }
}catch(e){}
        ...
        if (_routeEl && !_routeEl.textContent.trim()){
          ...
          setTimeout(()=>{
            try{
              if (_last === thisCs && _routeEl){
                const cur = _routeEl.textContent.trim();
                if (!cur || cur === "UNAVAILABLE" || cur === "unavailable") _set("EN ROUTE");
              }
            }catch(e){}
          }, 1800);
        }
        ...
        if (txt && _last === cs) {
          const rr = __fwFixArrow(txt);
          _set(rr);
          if (rr.includes("→")) _lastGoodRoute.set(cs, rr);
          ...
          try{
            ...
            if (!cur || cur === "EN ROUTE" || cur === "en route" || cur === "UNAVAILABLE" || cur === "unavailable") {
              _set("EN ROUTE");
            }
...
async function loadFlights(){
  ...
  const bb = fwGetBboxV118(HOME_LAT, HOME_LON);
...
function start(){
  document.getElementById("fw-version").textContent = "v121";
  hookControls();
