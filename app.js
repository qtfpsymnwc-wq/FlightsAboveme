...
function render(){
  ...
  if (window.__FW_UPDATE_AIRCRAFT__ && (Date.now()-__fwStableSince)>=FW_STABLE_MS) window.__FW_UPDATE_AIRCRAFT__();
}
...
async function loadFlights(){
  ...
  try{
    const k = String((current&&current.callsign)||"") + "|" + String((current&&current.icao24)||"");
  ...
  try{ if (window.__FW_FORCE_ROUTE__ && (Date.now()-__fwStableSince)>=FW_STABLE_MS) window.__FW_FORCE_ROUTE__(); }catch(e){}
  try{ if (window.__FW_UPDATE_AIRCRAFT__ && (Date.now()-__fwStableSince)>=FW_STABLE_MS) window.__FW_UPDATE_AIRCRAFT__(); }catch(e){}
  if (window.__FW_UPDATE_AIRCRAFT__ && (Date.now()-__fwStableSince)>=FW_STABLE_MS) window.__FW_UPDATE_AIRCRAFT__();
  if (window.__FW_UPDATE_ROUTE__) window.__FW_UPDATE_ROUTE__();
  try{ setStatusLine(`Radar: ${rawCount} flights • Showing: ${flights.length} • Loc: ${coordSource} (${HOME_LAT.toFixed(3)}, ${HOME_LON.toFixed(3)})`); }catch(e){}
...
function start(){
  document.getElementById("fw-version").textContent = "v124";
  hookControls();
