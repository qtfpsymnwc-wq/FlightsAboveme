...
function isCommercialCallsign(callsign){
  ...
  const cs = String(callsign).trim().toUpperCase();
  if (cs.startsWith('N')) return false;
  if (!/^[A-Z]{3}[0-9]+/.test(cs)) return false;
  const pref = cs.slice(0,3);
  return PASSENGER_ICAO_CODES.has(pref);
}
...
function start(){
  document.getElementById("fw-version").textContent = "v87";
  hookControls();
...
function updateLastUpdate(){
  ...
  const elInline = document.getElementById("lastUpdateInline");
  const dot = document.getElementById("liveIndicator");
  const d = new Date();
  const txt = d.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit', second:'2-digit'});
  if (el) el.textContent = "LAST UPDATE: " + txt;
