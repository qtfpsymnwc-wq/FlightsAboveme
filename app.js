...
P00; // 10s stable closest before route lookups
...
// v136: adaptive radar polling to avoid OpenSky 429
...
J00;   // v127: 5s stable closest before aircraft lookups
...
/* v136: Tap-to-select + faster enrichment + show registration */
window.__FW_PIN__ = window.__FW_PIN__ || { hex:"", callsign:"", ts:0 };
(function(){
  function normCS(cs){ return String(cs||"").trim().toUpperCase(); }
  function bindBox(id){
    const el = document.getElementById(id);
    if (!el) return;
    el.style.cursor = "pointer";
    el.addEventListener("click", () => {
      const hex = (el.dataset && el.dataset.hex) ? String(el.dataset.hex).trim().toLowerCase() : "";
      const cs  = (el.dataset && el.dataset.callsign) ? normCS(el.dataset.callsign) : "";
      if (!hex && !cs) return;
      window.__FW_PIN__ = { hex, callsign: cs, ts: Date.now() };
      const st = document.getElementById("status");
      if (st) st.textContent = `Pinned: ${cs || hex}`;
    });
  }
  ["sec1","sec2","sec3","sec4"].forEach(bindBox);

  // Override primary selection when pinned flight is present in the current list
  window.__fwPickPinnedFlight = function(flights){
    try{
      const pin = window.__FW_PIN__;
      if (!pin || (!pin.hex && !pin.callsign)) return null;
      // auto-expire pin after 10 minutes
      if (pin.ts && (Date.now() - pin.ts) > 10*60*1000) return null;
      if (!Array.isArray(flights)) return null;
      if (pin.hex){
        const h = pin.hex.toLowerCase();
        const hit = flights.find(f => String(f?.icao24||"").toLowerCase() === h);
        if (hit) return hit;
      }
      if (pin.callsign){
        const c = normCS(pin.callsign);
        const hit = flights.find(f => normCS(f?.callsign) === c);
        if (hit) return hit;
      }
      return null;
    }catch(e){ return null; }
  ...
  // Helper to render registration when aircraft enrichment exists
  window.__fwRenderAircraftExtra = function(ac){
    const reg = ac && (ac.registration || ac.reg);
    const m2 = document.getElementById("model2");
    if (m2){
      m2.textContent = reg ? `REG ${String(reg).trim()}` : "";
    }
  };
})();
...
function start(){
  document.getElementById("fw-version").textContent = "v136";
  hookControls();
