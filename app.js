...
function render(){
  ...
  if (!current){
    ...
    setText("badge","≈");
    if (logoEl) setHTML("logo", logoSVGFromCallsign(""));
    return;
  }

  setText("cs",(current.callsign || "UNKNOWN"));
  if (logoEl) setHTML("logo", logoSVGFromCallsign(current.callsign));
...
function start(){
  document.getElementById("fw-version").textContent = "v55";
  hookControls();
