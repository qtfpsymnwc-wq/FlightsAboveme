...
const FW_STABLE_MS = 10000; // 10s stable closest before route/aircraft lookups
...
function __fwIsStable(){
  return (Date.now() - (__fwStableSince||0)) >= FW_STABLE_MS;
}
...
try{ window.__FW_FORCE_ROUTE_RAW__ = update; window.__FW_FORCE_ROUTE__ = ()=>{ try{ if(__fwIsStable()) return window.__FW_FORCE_ROUTE_RAW__(); }catch(e){} }; }catch(e){} // v125_wrap_force_route
})();
...
(function(){
  ...
  window.__FW_UPDATE_AIRCRAFT__ = ()=>{ try{ if(__fwIsStable()) return window.__FW_UPDATE_AIRCRAFT_RAW__(); }catch(e){} }; // v125_wrap_aircraft
...
function start(){
  document.getElementById("fw-version").textContent = "v125";
  hookControls();
