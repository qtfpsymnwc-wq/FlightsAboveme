...
// v90: closest-flight route lookup (AeroData via Cloudflare Worker proxy)
// Set this to your Worker origin (no trailing slash). Example:
//   https://flightsabove.t2hkmhgbwz.workers.dev
const AERO_PROXY_ORIGIN = "https://flightsabove.t2hkmhgbwz.workers.dev";

const ROUTE_CACHE_TTL_MS = 10 * 60 * 1000; // 10 min
...
function cacheGet(key){
  ...
  if (!it) return null;
  if (nowMs() - it.ts > ROUTE_CACHE_TTL_MS) { routeCache.delete(key); return null; }
  return it.data;
}

function cacheSet(key, data){
  routeCache.set(key, {ts: nowMs(), data});
}

function buildRouteCandidates(callsign, icao24){
  const cs = encodeURIComponent(String(callsign||"").trim());
  const hx = encodeURIComponent(String(icao24||"").trim());
  const base = String(AERO_PROXY_ORIGIN||"").replace(/\/$/, "");
  const out = [];
  if (base){
    // Try common patterns (we'll use the first that works)
    if (callsign) out.push(`${base}/aero/${cs}`);
    if (callsign) out.push(`${base}/aero/route/${cs}`);
    if (callsign) out.push(`${base}/aero/callsign/${cs}`);
    if (callsign) out.push(`${base}/aero?callsign=${cs}`);
    if (icao24) out.push(`${base}/aero?icao24=${hx}`);
    if (icao24) out.push(`${base}/aero/icao24/${hx}`);
  }
  return out;
}

function normalizeRouteResponse(j){
  if (!j || typeof j !== "object") return null;

  // If the API already gives a nice route string
  if (typeof j.route === "string" && j.route.trim()) {
    return { text: j.route.trim(), origin: j.origin || null, destination: j.destination || null, raw: j };
  }

  // Standard shapes: {origin:{iata,icao,city}, destination:{...}}
  const o = j.origin || j.departure || j.from || null;
  const d = j.destination || j.arrival || j.to || null;

  function fmtEnd(e){
    if (!e) return "";
    const city = (e.city || e.municipality || e.name || "").toString().trim();
    const code = (e.iata || e.icao || "").toString().trim();
    if (city && code) return `${city} (${code})`;
    return city || code || "";
  }

  const left = fmtEnd(o);
  const right = fmtEnd(d);
  if (left && right) return { text: `${left} → ${right}`, origin: o, destination: d, raw: j };

  // Sometimes: {from:"KPHX", to:"KFAR"}
  if (typeof j.from === "string" && typeof j.to === "string") {
    return { text: `${j.from} → ${j.to}`, origin: o, destination: d, raw: j };
  }
...
async function fetchRouteForFlight(f){
  ...
  const icao24 = f.icao24 || f.hex || "";
  const key = (callsign || icao24 || "").trim().toUpperCase();
  if (!key) return null;
  ...
  if (nowMs() < routeBackoffUntil) return null;

  const urls = buildRouteCandidates(callsign, icao24);
  for (const url of urls){
    try{
      const res = await fetch(url, { method:"GET", cache:"no-store" });
      if (res.status === 429){
        // Back off for 2 minutes
        routeBackoffUntil = nowMs() + 2*60*1000;
        return null;
      }
      if (!res.ok) continue;
      const j = await res.json();
      const norm = normalizeRouteResponse(j);
      if (norm){
        cacheSet(key, norm);
        return norm;
      }
...
function render(){
  try{ applyFiltersAndLimit();
  updateLastUpdate(); }catch(e){}
...
function start(){
  document.getElementById("fw-version").textContent = "v90";
  hookControls();
...
function applyFiltersAndLimit(){
  let out = flights || [];
  if (COMMERCIAL_ONLY){
    out = out.filter(f => isCommercialCallsign(f.callsign));
  }
