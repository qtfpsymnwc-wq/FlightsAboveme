...
(function(){
  ...
  async function updateAircraftForCurrent(){
    try{
      ...
      if (fmt && normalizeHex((window.__FW_CURRENT_FLIGHT||{}).icao24) === key) {
...
if (!window.__FW_ROUTE_WIRED__) {
  ...
  (function(){
    ...
    const ROUTE_REFRESH_MIN_MS = 60 * 1000;
...
function _now(){ return Date.now(); }
    ...
    async function update(){
      try{
        ...
        if (_canFetch) _routeFetchGate.set(cs, Date.now());
        ...
        let txt = null;
        if (_canFetch) txt = await _fetch(cs);
        if (txt && _last === cs) {
...
function start(){
  document.getElementById("fw-version").textContent = "v117";
  hookControls();
