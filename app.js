...
function isCommercialCallsign(callsign){
  ...
  if (!pref || pref.length !== 3) return false;
  if (typeof PRIVATE_PREFIXES !== "undefined" && PRIVATE_PREFIXES.has(pref)) return false;
  return (typeof COMMERCIAL_CARRIER_CODES !== "undefined") && COMMERCIAL_CARRIER_CODES.has(pref);
}
...
function restartRotate(){
  ...
  // Reset to closest on any toggle change
...
function start(){
  document.getElementById("fw-version").textContent = "v79";
  hookControls();
...
// v76: toggles (commercial + scroll5)
(function(){
  const c = document.getElementById("commercialOnly");
  const s = document.getElementById("scroll5");
  function sync(){
    if (c) COMMERCIAL_ONLY = !!c.checked;
    if (s) SCROLL5 = !!s.checked;
    restartRotate();
  }
