...
// v133: adaptive radar polling to avoid OpenSky 429
...
let PROXY = "https://flightsabove.t2hkmhgbwz.workers.dev/";
// v133: auto-detect best Worker origin (no manual edits)
const __FW_FALLBACK_PROXY__ = PROXY;
let __FW_PROXY_ORIGIN__ = (PROXY.endsWith("/") ? PROXY.slice(0,-1) : PROXY);
let _AERO_ORIGIN = __FW_PROXY_ORIGIN__;
let AERO_PROXY_ORIGIN = __FW_PROXY_ORIGIN__;

function __fwNormBase(x){
  try{ const u=new URL(String(x)); return u.origin; }catch(e){ return ""; }
}
async function __fwProbe(origin){
  const o = __fwNormBase(origin);
  if(!o) return null;
  const ctrl = new AbortController();
  const t = setTimeout(()=>ctrl.abort(), 2200);
  try{
    const r = await fetch(o + "/health", { cache:"no-store", signal: ctrl.signal });
    if(!r.ok) return null;
...
(async function(){
  let last="";
  try{ last = localStorage.getItem("fw.proxy") || ""; }catch(e){}
  const cands = [location.origin, last, __FW_FALLBACK_PROXY__].filter(Boolean);
  for (const c of cands){
    try{
      const ok = await __fwProbe(c);
      if(ok){
        __FW_PROXY_ORIGIN__ = ok;
        PROXY = ok + "/";
        _AERO_ORIGIN = ok;
        AERO_PROXY_ORIGIN = ok;
...
function start(){
  document.getElementById("fw-version").textContent = "v133";
  hookControls();
