...
function updateTicker(){ /* v67: ticker removed */ }
...
function render(){
  ...
  try{ updateSecondaryBoxes(queue, currentIndex); }catch(e){}
  if (logoEl){
...
function restartRotate(){
  ...
  // Always (re)render immediately from the best current queue.
  try{ renderCurrent(); }catch(e){}
  if (ROTATE_MODE === "rotate5"){
    rotateTimer = setInterval(()=>{ rotateStep(); }, ROTATE_MS);
  }
}
...
function start(){
  document.getElementById("fw-version").textContent = "v67";
  hookControls();
...
// v67: dropdown controls (range + mode)
(function(){
  ...
  function applyMode(){
    if (!ms) return;
    ROTATE_MODE = ms.value; // 'closest' or 'rotate5'
    restartRotate();
  }
  if (rs) rs.addEventListener("change", ()=>{ applyRange(); restartRotate(); });
...
// v67: show next 4 flights as boxes under main card (no ticker)
function updateSecondaryBoxes(queue, currentIndex){
  ...
  for (let i=0;i<4;i++){
    ...
    if (!queue || queue.length < 2){
      el.querySelector(".secCS").textContent = "—";
      el.querySelector(".secRoute").textContent = "—";
      continue;
    }
    const f = queue[(currentIndex + 1 + i) % queue.length];
    const cs = (f && f.callsign) ? f.callsign : "—";
    const from = (f && f.origin && (f.origin.iata || f.origin.icao)) ? (f.origin.iata || f.origin.icao) : "";
    const to = (f && f.destination && (f.destination.iata || f.destination.icao)) ? (f.destination.iata || f.destination.icao) : "";
    const route = (f && typeof f.route === "string" && f.route) ? f.route : (from && to ? `${from} → ${to}` : (from || to || "—"));
