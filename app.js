...
function restartRotate(){
  ...
  rotateTimer = setInterval(()=>{
    ...
    if (key !== lastDisplayedKey){ lastDisplayedKey = key;  }
    render();
  }, rotateEveryMs);
}
...
async function tick(){
  ...
  try{
    ...
    const key = (current && current.callsign) ? current.callsign : String(currentIndex);
    if (key !== lastDisplayedKey){ lastDisplayedKey = key; if (hasRenderedOnce)  }
...
function start(){
  document.getElementById("fw-version").textContent = "v51";
  hookControls();
