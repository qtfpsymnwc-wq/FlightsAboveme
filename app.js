...
function updateTicker(){
  ...
  if (!t) return;
  if (!flights.length){
    const html = `<span class="tickerSpan">NO TRAFFIC IN RANGE</span><span class="tickerSpan">NO TRAFFIC IN RANGE</span>`;
    if (t.innerHTML !== html){ t.innerHTML = html; t.classList.add("fw-seamless"); }
    return;
  }
  const items = flights.slice(0,5).map(f => {
    ...
    const op = airlineCodeFromCallsign(f.callsign);
    const parent = parentBrandCodeFromPrefix(op);
    const name = parentBrandNameFromCode(parent);
    const sub = regionalSubLabel(op, parent);
    const shown = sub && sub.trim() ? (name + " " + sub.split("•")[0].trim()) : name;
    return `${f.callsign || "—"}${shown ? (' ('+shown+')') : ''} • ${d} • ${a} • ${s}`;
  });
  const txt = items.join("   ✦   ");
  const html = `<span class="tickerSpan">${txt}</span><span class="tickerSpan">${txt}</span>`;
  if (t.innerHTML !== html) { t.innerHTML = html; t.classList.add("fw-seamless"); }
}
...
function start(){
  document.getElementById("fw-version").textContent = "v65";
  hookControls();
...
// v64: range UI (highlight ANY + hint)
