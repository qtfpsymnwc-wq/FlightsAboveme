...
(function(){
  ...
  function formatAircraft(j){
    ...
    // Support either worker-wrapped response {ok:true, aircraft:{...}} OR direct aircraft payload.
    const a = j.aircraft || j;
    const manufacturer = clean(a.manufacturer || a.manufacturerName || "");
    const model = clean(a.model || a.modelName || a.typeName || "");
    const icaoType = clean(a.icaoType || a.icaoTypeDesignator || a.type || a.icao || "");
    const reg = clean(a.reg || a.registration || "");

    // Build best readable line
    let primary = "";
    if (manufacturer && model) primary = `${manufacturer} ${model}`;
    else primary = model || manufacturer || "";

    if (icaoType && primary && !primary.includes(icaoType)) primary = `${primary} (${icaoType})`;
    else if (icaoType && !primary) primary = icaoType;

    if (reg && primary && primary.length < 32) {
      // Keep reg lightweight; don't overcrowd
      // Example: "Embraer E175 (E75L) • N123AB"
      primary = `${primary} • ${reg}`;
    }
    return primary || null;
  }
  ...
  async function updateAircraftForCurrent(){
    try{
      ...
      if (key !== lastKey){
        ...
        setModelLine(radarType || "—");
      }

      const txt = await fetchAircraftByIcao24(key);
      if (txt && normalizeHex((window.__FW_CURRENT_FLIGHT||{}).icao24) === key) {
        setModelLine(txt);
      }
    }catch(e){
      // no-op
  ...
  window.__FW_UPDATE_AIRCRAFT__ = updateAircraftForCurrent;
})();
...
function _now(){ return Date.now(); }
    ...
    async function update(){
      try{
        const f = window.__FW_CURRENT_FLIGHT;
        const cs = (f && f.callsign) ? String(f.callsign).trim().toUpperCase() : "";
        ...
        if (cs !== _last){ _last = cs; _set("EN ROUTE"); }
        const txt = await _fetch(cs);
        if (txt && _last === cs) _set(txt);
        else if (_last === cs) _set("EN ROUTE");
      } catch(e) {
        _set("EN ROUTE");
      }
    }
...
async function loadFlights(){
  ...
  const dLat = milesToDegLat(queryRadiusMi);
  const dLon = milesToDegLon(queryRadiusMi, HOME_LAT);
...
function start(){
  document.getElementById("fw-version").textContent = "v113";
  hookControls();
