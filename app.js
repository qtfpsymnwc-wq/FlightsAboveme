// FlightWall Ultimate (frontend)
// - Auto-detect location
// - Commercial airliners only
// - Stacked queue + single rotating highlight
// - Route + aircraft model via your Cloudflare Worker proxy
//
...
let HOME_LAT = null;
let HOME_LON = null;
const RANGE = 0.6;        // ~70km-ish box
const OVERHEAD_KM = 1.2;  // overhead alert threshold
let flightsCache = [];
let index = 0;

const PROXY = "https://flightsabove.t2hkmhgbwz.workers.dev/";

// Airline mapping (ICAO prefix) + AirHex logos
const AIRLINES = {
  ...
  // Add more here as you want (ICAO prefix → name/logo)
};
...
async function lookupFlight(callsign) {
  const cs = cleanCallsign(callsign);
  if (!cs) return { route: "Unavailable", model: "Unknown", callsign: cs };

  try {
    const res = await fetch(PROXY + "adsb/" + encodeURIComponent(cs));
...
async function renderFlight() {
  ...
  current.innerHTML = `
    ...
    <div class="flight-route">${extra.route || "Unavailable"} • ${extra.model || "Unknown"}</div>
